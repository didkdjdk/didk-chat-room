...
import com.didk.commons.tools.validator.group.AliyunGroup;
import com.didk.commons.tools.validator.group.QcloudGroup;
import com.didk.commons.tools.validator.group.QiniuGroup;
...
public class SmsController {
    ...
    @LogOperation("保存")
    @PreAuthorize("hasAuthority('sys:sms:all')")
    public Result save(@RequestBody SysSmsDTO dto) throws Exception {
        //校验数据
    ...
    @LogOperation("修改")
    @PreAuthorize("hasAuthority('sys:sms:all')")
    public Result update(@RequestBody SysSmsDTO dto) throws Exception {
        //校验数据
        if (dto.getPlatform() == PlatformEnum.ALIYUN.value()) {
            //校验阿里云数据
            ValidatorUtils.validateEntity(dto.getConfig(), AliyunGroup.class);
