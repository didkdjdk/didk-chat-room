...
import com.didk.commons.tools.enums.SuperAdminEnum;
...
import com.didk.commons.tools.utils.TreeUtils;
import com.didk.dao.SysDeptDao;
import com.didk.dao.SysUserDao;
import com.didk.model.dto.SysDeptDTO;
import com.didk.model.entity.SysDeptEntity;
import com.didk.service.SysDeptService;
...
public class SysDeptServiceImpl extends BaseServiceImpl<SysDeptDao, SysDeptEntity> implements SysDeptService {
    ...
    public void update(SysDeptDTO dto) {
        ...
        if (entity.getId().equals(entity.getPid())) {
            throw new ZException(ErrorCode.SUPERIOR_DEPT_ERROR);
        }
    ...
    public void delete(Long id) {
        ...
        if (subList.size() > 1) {
            throw new ZException(ErrorCode.DEPT_SUB_DELETE_ERROR);
        }

        //判断部门下面是否有用户
        int count = sysUserDao.getCountByDeptId(id);
        if (count > 0) {
            throw new ZException(ErrorCode.DEPT_USER_DELETE_ERROR);
        }
