...
import com.didk.oauth.model.entity.GitLabUserInfo;
...
public class GitLabOAuthServiceImpl implements OAuthService {
    ...
    public OAuthUserInfo getUserInfo(String code) {
        ...
        GitLabUserInfo userInfo = webClient.get()
                .uri(getClientProperties().getUserInfoUrl())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken)
                .retrieve()
                .bodyToMono(GitLabUserInfo.class)
                .doOnError(e -> log.error("获取GitLab用户信息失败", e))
                .block();

        Assert.notNull(userInfo, "获取GitLab用户信息失败");
        return userInfo;
    }
    ...
    private String getAccessToken(String code) {
        ...
        Map<String, Object> response = webClient.post()
                ...
                .doOnError(e -> log.error("获取GitLab AccessToken失败", e))
                .block();

        Assert.notNull(response, "获取GitLab AccessToken失败");
